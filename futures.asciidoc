...
=== Preventing synchronous errors in async code

==== Problem

You are writing a function that returns a Future. Your function contains
synchronous code that could potentially throw. Clients of this function are
forced to implement dual error handlers: `catchError()` for asynchronous errors,
and a second handler for synchronous errors. You want to prevent the need for
multiple error handlers.

Let us look at an example of how synchronous errors could easily leak out of a
...
Two functions in the code above could potentially throw synchronously:
...
throws, a synchronous error gets thrown:
...
In the abscence of any synchronous error handler, the error propagates to the
...
Wrap the function body in a new Future. This ensures that no synchronous error
leaks out of your code:
...
The old function code is now the callback passed to the `new Future()`
constructor. Running your code produces the following results:

* If the callback returns a non-Future value, `new Future()`’s Future completes
with that value.

* If the callback throws, `new Future()`'s Future completes with an error.

* If the callback itself returns a Future, the value or the error of that
Future completes `Future()`'s Future.

With your old code wrapped within a `new Future()`, `catchError()` can handle
all errors:
...
The output from running the code above looks something like this:

--------------------------------------------------------------------------------
Error: <error from obtainFilename>
--------------------------------------------------------------------------------

Wrapping your code in a `new Future()` makes your code resilient against
uncaught exceptions. If your function has a lot of code packed into it,
chances are that you could be doing something dangerous without realizing it:

--------------------------------------------------------------------------------
Future myFunc() {
  return new Future(() {
    var x = someFunc();     // Unexpectedly throws in some rare cases.
    var y = 10 ~/ x;        // Throws if x is 0.
    ...
  });
}
--------------------------------------------------------------------------------

This approach allows you to handle errors you know might occur, but also
prevents errors from accidentally leaking out of your code.
