...
the cost of allocating memory for the new items just once. On the author's
MacBook, appending items in this manner works approximately twice as fast
as using `add()` or `addAll()`.
