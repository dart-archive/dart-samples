...
// Recipes talking about BMP: 
// converting between characters and numeric codes (this is the first one)
// get length
...
Using `codeUnitAt()` with the `clef` character leads to problems:
...
surrogate pair can never be mistaken for a single valid character.
...
characters it has. Anyone can tell that 'Dart' has four characters, and
'Amelié' has six, right? Almost. The length of 'Dart' is indeed four, but
...
For almost every string that you are likely to encounter, simple subscripting
using `[]` works fine:
...
is problematic. This is because the default `[]` implementation subscripts
...
If you are sure that the string is in the Basic Multilingual Plane (BMP), you can
use string.split(''):
