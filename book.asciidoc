...
You want to create a list but don't want anyone to be able to add or remove
items.
...
A `const` list is a compile time constant. You code will run faster if you use
a `const` list.
...
Or, you can increase the `length` property of a list, incur the cost of
allocating memory for the new items just once, and then add the new elements:
...
names.length += 2; // Space for 2 more items created.
...
Increasing the length of a list by `n` items allocates space for an additional
`n` items. The new items are initialized to `null`:

--------------------------------------------------------
print(names); // ['Seth', 'Timothy', 'John', null, null]
--------------------------------------------------------

Manipulating the length property is a useful optimization strategy: when
several items have to be appended, it is better to increase the list size just
once.
