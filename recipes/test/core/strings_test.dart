...
void main() {
  group("concatenating strings:", () {
    ...
    group("using concat()", () {
      test("", () {
        expect("Dart".concat(" is").concat( " fun!"), equals(string)); 
      });
      
      test("with identifier", () {
        expect(lang.concat(" is").concat( " fun!"), equals(string)); 
      });
    });
    
    test("using join()", () {
      expect(["Dart", "is", "fun!"].join(" "), equals(string));
    });
  });
  ...
  group("subscripting a string that contains symbols", () {
    var coffee = "\u{1F375}";
