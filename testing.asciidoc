...
You want to test exceptions in your code. Maybe you just want to know if some
code returns normally, or if it throws. Sometimes you want to test that a
specific error is raised, and sometimes you want to test that the error message
is what you intended. What support does the Unittest library have for testing
exceptions?

==== Solution

To demonstrate exception testing, we'll define a `range()` function that
generates a list of numbers in the range defined by its two arguments, and
throws if the first argument is greater or equal to the second
one:
...
To test whether a some code throws, use the `throws` matcher: 

------------------------------------------------------------------------------
expect(() => range(10, 5), throws);
------------------------------------------------------------------------------

To test that your runs without generating an exception, use the
...
You can test for a specific error:
...
You can also use `throwsA` with a matcher argument to test the exception type:
...
To test the type of an exception, and the error message, do the following:

------------------------------------------------------------------------------
expect(() => range(5, 3), throwsA(predicate((e) => (e is ArgumentError &&
    e.message == 'arg1 must be less than arg2'))));
------------------------------------------------------------------------------

Here is another way to test the type and the error message:
