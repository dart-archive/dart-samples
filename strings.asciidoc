...
xref:string-buffer[Incrementally building a string efficiently using a StringBuffer].
...
fragments into a new string until instructed to do so. The use of a StringBuffer
is the recommended way of accumulating string fragments and is more efficient
...
more. The example below shows how you can use these methods:
...
directly performing an indexing operation on an extended string:

-------------------------------------------------------------------------------
print(clef[0]); // Invalid string. Dart Editor prints '?'
-------------------------------------------------------------------------------

Instead, use a string's `runes` list with the `[]` operator:
...
See the
xref:numerical-codes[Converting between characters and numerical codes])
recipe for a discussion on how to convert between numerical codes and strings
that may contain extended characters.

See the
xref:calculating-length[Calculating the length of a string] recipe for a
discussion on how to calculate the length of a string that may contain extended
characters.
...
an acute accent ('\u0301'). The `anotherName` variable is defined using these
...
are expressed as fractions (2/7, 4/5, etc,)  or as integer divisions (3~/5). A
RegExp splits the string to extract just the numerical values.
...
The RegExp matches the middle word ('SHOOTS'). A pair of callbacks that you
register transform the matched and unmatched substrings before the substrings
are joined together again. In the example shown, the matched strings are
converted to lowercase, and the unmatched strings are converted to uppercase.
...
You can use `string.indexOf()`, which returns -1 if the substring is not found
within a string, and otherwise returns the matching index:
...
In the example above, `hasMatch()` returns `true` since the strings 'are' and
...
Use the RegExp factory constructor to create a regular expression:
...
special characters have no special meaning (See
...
* The '\' in the '\d' is escaped using another '\'.

Using a raw string obviates the need to escape the '\' in '\+' and '\d'.

The code below uses the regular expression defined above with the
`stringMatch()` method to get a matching substring from within a longer string:
...
To get all matches, use `allMAtches()`:
...
Using `replaceFirst()` replaces the leading '0' with an empty string.
